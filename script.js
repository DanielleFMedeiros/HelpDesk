...
function obterFabricanteDoMacPHP(mac) {
  const xhr = new XMLHttpRequest();
  xhr.open('GET', `macvendor.php?mac=${mac}`, true);

  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        const fabricante = xhr.responseText;
        document.getElementById('resultado').textContent = fabricante;
      } else {
        console.error('Erro na solicitação ao servidor. Código de status:', xhr.status);
        document.getElementById('resultado').textContent = 'Erro na solicitação ao servidor.';
      }
    }
  };
